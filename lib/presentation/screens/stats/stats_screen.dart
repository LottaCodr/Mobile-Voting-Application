import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:mobile_voting_application/presentation/widgets/bar_chart.dart';
import 'package:mobile_voting_application/presentation/widgets/election_card.dart';
import 'package:mobile_voting_application/presentation/widgets/selection_card.dart';
import 'package:mobile_voting_application/data/models/candidate_model.dart';
import 'package:mobile_voting_application/core/theme/colors.dart';
import 'package:mobile_voting_application/presentation/providers/election_controller.dart';

class StatsScreen extends ConsumerStatefulWidget {
  const StatsScreen({super.key});

  @override
  ConsumerState<StatsScreen> createState() => _StatsScreenState();
}

class _StatsScreenState extends ConsumerState<StatsScreen> {
  List<String> pickELection = ['Presidential', 'Governorship', "Senatorial"];
  List<String> pickRegion = ['National', 'State'];
  String selectedElection = 'Presidential';
  String selectedRegion = 'National';

  @override
  Widget build(BuildContext context) {
    final selectedCandidates = ref.watch(electionProvider);
    final notifier = ref.read(electionProvider.notifier);
    return Scaffold(
      appBar: AppBar(
        title: const Text("Vote Statistics"),
        centerTitle: true,
        bottom: PreferredSize(
          preferredSize: const Size.fromHeight(70.0),
          child: Padding(
            padding: const EdgeInsets.all(14),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                SelectionCard(
                  header: 'Pick Election',
                  selected: selectedElection,
                  electionSelected: pickELection,
                  textColor: MVAColors.primaryColor,
                  bgColor: MVAColors.primaryColor.withOpacity(0.2),
                  textSize: 12,
                  dropDownWidth: 150,
                ),
                SelectionCard(
                  header: 'Region',
                  selected: selectedRegion,
                  electionSelected: pickRegion,
                  textColor: MVAColors.primaryColor,
                  bgColor: MVAColors.primaryColor.withOpacity(0.2),
                  textSize: 12,
                  dropDownWidth: 150,
                ),
              ],
            ),
          ),
        ),
      ),
      body: SingleChildScrollView(
        child: Padding(
          padding: const EdgeInsets.all(8),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const SizedBox(height: 10),
              Container(
                width: MediaQuery.of(context).size.width,
                height: 400,
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(10),
                  color: MVAColors.backgroundColor,
                ),
                child: BarChart(
                  electionType: selectedElection,
                ),
              ),
              const SizedBox(height: 40),
              Padding(
                padding: const EdgeInsets.all(8.0),
                child: Text(
                  'Candidates',
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18),
                ),
              ),
              Container(
                padding: const EdgeInsets.only(bottom: 25),
                height: 400,
                width: MediaQuery.of(context).size.width,
                child: ListView.builder(
                  physics: const ScrollPhysics(),
                  scrollDirection: Axis.vertical,
                  itemCount: myCandidates.length,
                  itemBuilder: (context, index) {
                    final candidate = myCandidates[index];
                    final isVoted = selectedCandidates[candidate.electionId] == candidate.id;
                    return ElectionCard(
                      candidate: candidate,
                      isVoted: isVoted,
                      onVoteToggle: () {
                        notifier.selectCandidate(candidate.electionId, candidate.id);
                      },
                    );
                  },
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
